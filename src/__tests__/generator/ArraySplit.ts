...
export function ArraySplitsGen(length:number, minSplits = -1, maxSplits = -1):Generator<ArraySplit[]> {
    ...
    return interval(minSplits, maxSplits).flatMap(numSplits => {
        if(numSplits <= 1)
            return just<ArraySplit[]>([{from: 0, length}])

        const seqGen = SetGen(interval(1, length-1), numSplits-1, numSplits-1).map(set => getSortedArrayFromSet(set))
        ...
        return seqGen.map(seq => {
            ...
            if(seq[0] <= 0)
                throw new Error(`invalid length: ${JSONStringify(seq)} at ${-1}`)

            for(let i = 0; i < seq.length - 1; i++) {
                arr.push({from: seq[i], length: seq[i+1]-seq[i]})
                if(seq[i+1]-seq[i] <= 0)
                    throw new Error(`invalid length: ${JSONStringify(seq)} at ${i}`)
            }
