...
export function AttributeMapGen(
    allowNullAttr = true,
    strGen: Generator<string> = PrintableASCIIStringGen(0, 3),
): Generator<IOrB> {
    return interval(0, allowNullAttr ? 7 : 2).flatMap<IOrB>(kind => {
        switch (kind) {
            case 0:
                return strGen.map<IOrB>(str => {
                    return { b: str }
                })
            case 1:
                return strGen.map<IOrB>(str => {
            ...
            case 2:
                return TupleGen(strGen, strGen).map<IOrB>(tuple => {
                    return { b: tuple[0], i: tuple[1] }
                })
            case 3:
