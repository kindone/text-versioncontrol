...
export function OpsGen(baseLength = -1, withEmbed = true, withAttr = true): Generator<Op[]> {
    const baseLengthGen: Generator<number> =
        baseLength == -1 ? oneOf(weightedGen(inRange(1, 3), 0.9), inRange(3, 20)) : just(baseLength)
            ...
            return baseOpsGen.flatMap(baseOps => {
                ...
                return interval(0, baseOps.length + 1).flatMap(numInserts => {
                    ...
                    return TupleGen(insertPositionsGen, insertsGen).map(tuple => {
                        ...
                        expect(minContentLengthForChange(new Delta(baseOps))).toBe(
                            minContentLengthForChange(new Delta(resultOps)),
                        )
                        return resultOps
                    })
                })
