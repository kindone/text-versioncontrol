...
export const AttributeMapGen = (() => {
    const generator = jsc.generator.bless(
        (size: number): AttributeMap => {
    ...
    const shrink = jsc.shrink.bless(
        (attrs: AttributeMap): AttributeMap[] => {
...
export const OpGen = (() => {
    const generator = jsc.generator.bless(
        (size: number): Op => {
            ...
            switch (jsc.random(0, 2)) {
                case 0:
                    return { retain: jsc.nat.generator(size) + 1, attributes }
                case 1:
                    return { delete: jsc.nat.generator(size) + 1, attributes }
                case 2:
                    return { insert: jsc.asciinestring.generator(size), attributes }
            }
        },
    )
    ...
    const shrink = jsc.shrink.bless(
        (op: Op): Op[] => {
            const size = op.insert ? op.insert.toString().length : op.retain ? op.retain : op.delete
