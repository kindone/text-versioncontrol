...
import { Action, actionGenOf, interval, just, oneOf, PrintableASCIIStringGen, statefulProperty, TupleGen } from "jsproptest";
...
describe('Document', () => {
    it('clone', () => {
        ...
        const cloneActionGen = just(new Action((doc:Document, _:DocumentModel) => {
            expectEqual(doc, doc.clone())
        }))

        const actionGen = actionGenOf(cloneActionGen)

        const prop = statefulProperty(docGen, (document:Document) => new DocumentModel(contentLength(document.getContent())), actionGen)
        prop.go()
    })

    it('append', () => {
        const docGen = DocumentInitialGen('doc')
        const appendActionGen = TupleGen(interval(0, 100), interval(0, 20)).flatMap(tuple => ChangeListGen(tuple[0], tuple[1])).map(changeList => new Action((doc:Document, model:DocumentModel) => {
        ...
        const prop = statefulProperty(docGen, (document:Document) => new DocumentModel(contentLength(document.getContent())), actionGen)
        prop.setMaxActions(10).setNumRuns(10).go()
