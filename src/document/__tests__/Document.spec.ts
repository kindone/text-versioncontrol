...
describe('document', () => {
    it('take0-1', () => {
        ...
        expectEqual(doc.take(0, 1), { ops: [{ insert: 'a' }] })
    })

    it('take0-0', () => {
        const doc = new Document('doc1', 'a')

        expectEqual(doc.take(0, 0), { ops: [] })
    })

    it('take0-len', () => {
        const doc = new Document('doc1', 'a')

        expectEqual(doc.take(0, contentLength(doc.getContent())), doc.getContent())
    })

    it('take empty', () => {
        const doc = new Document('doc1', '')

        expectEqual(doc.take(0, 0), doc.getContent())
    })
    ...
    it('take0-len+1', () => {
        const doc = new Document('doc1', 'a')
        expect(() => {
            doc.take(0, contentLength(doc.getContent()) + 1)
    ...
    it('document.take equals range.crop', () => {
        // return
        forAll(
            (content: IDelta, num1: number, num2: number) => {
                // console.log(content)
