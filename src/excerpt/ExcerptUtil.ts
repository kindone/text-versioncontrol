...
export class ExcerptUtil {
    ...
    public static makeExcerptMarker(
        markedAt: 'left' | 'right',
        sourceUri: string,
        sourceRev: number,
        sourceStart: number,
        ...
        if (targetEnd < 0) targetEnd = targetStart + sourceEnd - sourceStart + 1 // marker itself is included
        const attributes = {
            markedAt,
            targetUri,
            targetRev: targetRev.toString(),
            targetStart: targetStart.toString(),
            targetEnd: targetEnd.toString(),
        }

        const op = { insert: value, attributes }

        if (!ExcerptUtil.isExcerptMarker(op)) throw new Error('error: ' + JSONStringify(op))
        return op
    }

    // target ranges: marker itself is included
    public static getPasteWithMarkers(
        source: ExcerptSource,
        targetUri: string,
        targetRev: number,
        targetStart: number,
    ): IDelta {
        const leftMarkerOp: Op = this.makeExcerptMarker(
            'left',
            source.uri,
            source.rev,
            source.start,
        ...
        const rightMarkerOp: Op = this.makeExcerptMarker(
            'right',
            source.uri,
            source.rev,
            source.start,
        ...
        if (!ExcerptUtil.isExcerptMarker(rightMarkerOp))
            throw new Error(
                'Unexpected error. Check marker and checker implementation: ' + JSONStringify(rightMarkerOp),
            )
    ...
    public static isLeftExcerptMarker(op: Op, includeCopied = false): boolean {
        if (!this.isExcerptMarker(op, includeCopied)) return false
    ...
    public static isExcerptMarker(op: Op, includeCopied = false): boolean {
        ...
        if (!insert.hasOwnProperty('excerpted') || !attributes || typeof insert.excerpted !== 'string') return false
        // filter out copied
        if (!includeCopied && attributes.hasOwnProperty('copied')) return false

        if (!ExcerptUtil.isExcerptURI(insert.excerpted)) return false

        return (
            typeof attributes.targetUri === 'string' &&
    ...
    public static decomposeMarker(op: Op) {
        if (!this.isExcerptMarker(op)) throw new Error('Given op is not a marker: ' + JSONStringify(op))
        ...
        return new Excerpt(
            { type: 'excerpt', uri: sourceUri, rev: sourceRev, start: sourceStart, end: sourceEnd },
            new ExcerptTarget(targetUri, targetRev, targetStart, targetEnd),
        )
    }

    public static splitSource(source: string) {
        if (!ExcerptUtil.isExcerptURI(source)) throw new Error('unsupported value: ' + source)

        const [sourceUri, rest] = source.split('?')

        const result = /^rev=([0-9]+)&start=([0-9]+)&end=([0-9]+)$/.exec(rest)
        if (!result) throw new Error('unsupported value: ' + source)
    ...
    public static getFullExcerpts(content: IDelta): Array<{ offset: number; excerpt: Excerpt }> {
        ...
        for (const op of content.ops) {
            if (!op.insert) throw new Error('content is in invalid state: ' + JSONStringify(op))
                ...
                if (ExcerptUtil.isExcerptMarker(op)) {
                    ...
                    if (excerptedOp.attributes.markedAt === 'left') {
                        ...
                        if (excerptMap.has(key)) {
                            ...
                            if (
                                marker.attributes.targetUri === excerptedOp.attributes.targetUri &&
    ...
    public static getPartialExcerpts(content: IDelta) {
        ...
        for (const key of Array.from(anyExcerpts.keys())) {
            if (!fullExcerpts.has(key)) partialExcerpts.push(anyExcerpts.get(key))
        }
