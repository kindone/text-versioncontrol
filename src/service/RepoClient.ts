...
export class RepoClient {
    ...
    constructor(docNames: string[]) {
        for (const docName of docNames) this.docs[docName] = new DocClient(new History(docName))
    }
    ...
    public sync(syncResponse: RepoSyncResponse) {
        for (const docName in syncResponse) {
            if (this.docs[docName]) this.docs[docName].sync(syncResponse[docName])
            else throw new Error('Cannnot sync. No document exists with the name: ' + docName)
        }
    }
    ...
    public getSyncRequest(): RepoSyncRequest {
        ...
        for (const docName in this.docs) {
            if (docName) repoSyncRequest[docName] = this.docs[docName].getSyncRequest()
        }
